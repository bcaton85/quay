...
export default function EditExpirationModal(props: EditExpirationModalProps) {
    ...
    useEffect(() => {
        if (successSetExpiration) {
            const dateMessage: string = isNullOrUndefined(date) ? "never" : formatDate(date.getTime() / 1000);
            const title: string = props.tags.length === 1 ? `Successfully set expiration for tag ${props.tags[0]} to ${dateMessage}` : `Successfully updated tag expirations to ${dateMessage}`;
    ...
    useEffect(() => {
        if (errorSetExpiration) {
            const title: string = props.tags.length === 1 ? `Could not set expiration for tag ${props.tags[0]}` : 'Could not update tag expirations';
            const errorDisplayMessage = (<>{Array.from(errorSetExpirationDetails.getErrors()).map(([tag, error]) => (<p key={tag}>Could not update expiration for tag {tag}: {error.error.message}</p>))}</>)
            addAlert({ variant: AlertVariant.Failure, title: title, message: errorDisplayMessage });
            props.setIsOpen(false);
            if(!isNullOrUndefined(props.onComplete)){
                props.onComplete();
            }
    ...
    const onSave = () => {
        const requestedDate = date === null || date === undefined ? null : date.getTime() / 1000;
                ...
                <DescriptionList>
                    <DescriptionListGroup>
                        ...
                        <DescriptionListDescription id="edit-expiration-tags">
                            {props.tags.map(tag => <Label key={tag}>{tag}</Label>)}
                    ...
                    <DescriptionListGroup>
                        ...
                        <DescriptionListDescription>
                                ...
                                <Button variant={ButtonVariant.secondary} onClick={() => setDate(null)}>Clear</Button>
                        </DescriptionListDescription>
                    </DescriptionListGroup>
                </DescriptionList>
            </Modal>
